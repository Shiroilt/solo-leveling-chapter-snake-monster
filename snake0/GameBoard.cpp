...
GameBoard::GameBoard(HighScoreManager* hsm, int difficulty, const string& name) :
    ...
    // Spawn each snake using its PlayerConfig — all per-player settings in one table
    for (int i = 0; i < NUM_PLAYERS; i++) {
        int spawnX = PLAYER_CONFIG[i].spawnXFraction * consoleWidth / 3;
        snakes[i] = new Snake(spawnX, consoleHeight / 2, 3, PLAYER_CONFIG[i].initialDirection);
    }
...
GameBoard::~GameBoard() {
    for (int i = 0; i < NUM_PLAYERS; i++) delete snakes[i];
}
...
void GameBoard::displayGameInfo() {
    // Player scores displayed at fixed positions — P1 left, P2 right of center
    static const int SCORE_XPOS[NUM_PLAYERS] = { 2, 20 };
    static const int SCORE_COLORS[NUM_PLAYERS] = { 10, 11 };
    for (int i = 0; i < NUM_PLAYERS; i++) {
        gotoxy(SCORE_XPOS[i], 0);
        setColor(SCORE_COLORS[i]);
        cout << "P" << (i + 1) << " Score: " << scores[i];
    }
    gotoxy(consoleWidth / 2, 0); setColor(13); cout << "Lvl: " << level;
    gotoxy(consoleWidth - 20, 0); setColor(10); cout << "Speed: " << currentSpeed << "ms";
    gotoxy(consoleWidth - 35, 0); setColor(6); cout << "Best: " << highScoreManager->getHighestScore();
}
...
void GameBoard::showLevelUpMessage() {
    ...
    for (int y = centerY - 1; y <= centerY + 2; y++) { gotoxy(centerX, y); cout << "                      "; }
}

void GameBoard::draw() {
    displayGameInfo();
    for (int i = 0; i < NUM_PLAYERS; i++) {
        snakes[i]->draw(PLAYER_CONFIG[i].headColor, PLAYER_CONFIG[i].bodyColor);
    }
    food.draw();
}
...
bool GameBoard::update() {
    ...
    for (int i = 0; i < NUM_PLAYERS; i++) {
        if (foodEaten[i]) {
            ...
            int oldLevel = level;
            updateLevel();
            if (level > oldLevel) { draw(); showLevelUpMessage(); draw(); }
            spawnFood();
        }
    }

    bool lost[NUM_PLAYERS] = {};
    for (int i = 0; i < NUM_PLAYERS; i++) {
    ...
    if (lost[0] && lost[1]) { loser = 3; gameOver = true; return false; }
    if (lost[0])             { loser = 1; gameOver = true; return false; }
    if (lost[1])             { loser = 2; gameOver = true; return false; }
...
void GameBoard::displayGameOver() {
    ...
    setColor(14);
    if (loser == 1)      cout << "   ❌ Player 1 Lost! (Player 2 Wins!)\n\n";
    else if (loser == 2) cout << "   ❌ Player 2 Lost! (Player 1 Wins!)\n\n";
    else                 cout << "   ❌ Both Players Collided! (Tie Game)\n\n";
    ...
    cout << "\n";
    setColor(11);
    if      (level >= 10) cout << "   🏆 LEGENDARY! You're Snake Masters!\n";
    else if (level >= 7)  cout << "   ⭐ EXCELLENT! Great skills!\n";
    else if (level >= 4)  cout << "   👍 GOOD JOB! Keep practicing!\n";
    else                  cout << "   💪 Nice try! You'll do better next time!\n";
